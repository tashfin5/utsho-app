  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editMode) {
        ...
        const res = await fetch(`${API_URL}/api/students/${currentId}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`
        ...
        fetchStudents(); // Refresh the list with updated data

      } else {
        // ✅ YOUR ORIGINAL CREATE LOGIC
        // 1. CREATE USER FIRST
        ...
        let studentData;
        try { studentData = await studentRes.json(); } catch { studentData = null; }
          ...
          {students.length === 0 ? (
            <div className="text-center mt-24 text-gray-400 flex flex-col items-center bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
              <div className="bg-red-50 p-4 rounded-full mb-3">
                <Users className="w-10 h-10 text-[#CC0000] opacity-80" />
              </div>
            ...
            students.map((student) => (
              ...
              >
                <div>
                  ...
                  <div className="text-[11px] text-gray-500 mt-1 font-medium space-x-2">
                    <span className="bg-gray-100 px-1.5 py-0.5 rounded text-gray-600">ID: {student.userId || "N/A"}</span>
                    <span className="bg-gray-100 px-1.5 py-0.5 rounded text-gray-600">Class {student.className}</span>
                    <span className="bg-gray-100 px-1.5 py-0.5 rounded text-gray-600">Roll: {student.rollNumber}</span>
                  </div>
                  <div className="text-[11px] text-gray-500 mt-1 font-medium space-x-2">
                    <span className="bg-gray-100 px-1.5 py-0.5 rounded text-gray-600">Phone Number: {student.phone}</span>
                  </div>
                </div>
            ...
            <div className="bg-white rounded-2xl w-full max-w-sm p-6 relative shadow-2xl">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-1 uppercase tracking-wide">Roll Number</label>
                    <input type="text" required className="w-full border border-gray-300 rounded-lg p-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#CC0000]/50 focus:border-[#CC0000] transition-colors"
                      value={formData.rollNumber} onChange={(e) => setFormData({...formData, rollNumber: e.target.value})} placeholder="e.g. 101" />
                  </div>
                </div>
